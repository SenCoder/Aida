...
type Controller interface {
	Excute(message common.Msg) []byte
}
...
func Route(pred interface{} ,controller Controller) {
	switch pred.(type) {
	case func(entry common.Msg)bool:{
		var arr [2]interface{}
		arr[0] = pred
		arr[1] = controller
	...
	case map[string]interface{}:{
		defaultPred:= func(entry common.Msg)bool{
...
func TaskDeliver(postdata []byte,conn net.Conn){
	for _ ,v := range routers{
		pred := v[0]
		act := v[1]
		var entermsg common.Msg
		err := json.Unmarshal(postdata,&entermsg)
		if err != nil {
			Log(err)
		}
		if pred.(func(entermsg common.Msg)bool)(entermsg) {
...
func (this *EchoController) Excute(message common.Msg)[]byte {
...
func init() {
	...
	Route(func(entry common.Msg)bool{
